...
class LeftNav extends Component {
  ...
  onSelect = ({ item, key, keyPath, selectedKeys, domEvent }) => {
    menuList.forEach(v => {
      if (v.key === key) {
        memoryUtils.title = v.title
        storageUtils.Savetitle(v.title)
      }
      if (v.children) {
        v.children.forEach(k => {
          if (k.key === key) {
            memoryUtils.title = v.title
            storageUtils.Savetitle(k.title)
          }
        })
      }
    })
  ...
  getMenuNodes = menuList => {
    ...
    return menuList.reduce((pre, item) => {
      if (!item.children) {
        ...
        const hasChild = item.children.find(cItem => cItem.key === path)
        if (hasChild) {
          this.setState({
            openItem: item.key
          })
...
//包装一个非路由组件，向新的组件传递history,location,match
